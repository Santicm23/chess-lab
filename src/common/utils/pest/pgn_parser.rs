...
pub fn parse_standard_pgn(input: &str) -> Game {
    ...
    for record in pair.into_inner() {
        match record.as_rule() {
            Rule::metadata => {
                ...
                let key = pairs.next().unwrap().get_input();
                let op_value = pairs.next();
                let value = if op_value.is_some() {
                    op_value.unwrap().get_input()
                } else {
                    ""
                };
            ...
            Rule::sequence => {
                for subsequence in record.into_inner() {
